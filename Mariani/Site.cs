...
public class Site : ISite
{
        ...
        using (var db = new MarianiContext(DomainConstraints.Connectionstring))
        {
            try
            {
                ...
                foreach (var item in ses)
                {
                    ...
                    validSessions.Add(new Session(item.Id, item.ValidUntil, item.ConnectionString, item.UserId, user, item.SiteId, AlarmClock, MinimumBidIncrement, SessionExpirationInSeconds));
                }
    ...
    public IEnumerable<IAuction> ToyGetAuctions(bool onlyNotEnded)
    {
        ...
        using (var db = new MarianiContext(DomainConstraints.Connectionstring))
        {
            ...
            var ac = db.Auctions.Include(a=>a.Session).ThenInclude(s=>s.Site).Where(a=>a.Session.Site.Name == Name).ToList();
            foreach (var auction in ac)
            {
                var user = db.Users.Single(u => u.Username == auction.SellerUsername);
                var a = new Auction(user, user.Username, auction.Description, auction.EndsOn, ConnectionString, 2, AlarmClock, auction.Price,
        ...
        using (var c = new MarianiContext(DomainConstraints.Connectionstring))
        {
            try
            {
                ...
                var ses = new Session(randomGen.Next(10000000).ToString(),
                    AlarmClock.Now.AddSeconds(SessionExpirationInSeconds), ConnectionString, owner.UserId, owner, owner.SiteId, AlarmClock, MinimumBidIncrement, SessionExpirationInSeconds);
                c.Sessions.Add(ses);
                c.SaveChanges();
                return ses;
            }
    ...
    public void CreateUser(string username, string password)
    {
        ...
        using (var c = new MarianiContext(DomainConstraints.Connectionstring))
        {
            try
            {
                ...
                var user = new User(username, hashpassword, ConnectionString, s.SiteId);
                c.Users.Add(user);
                c.SaveChanges();
            }
            catch (SqlException e)
